...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" integrity="sha512-iecdLmaskl7CVJkEZSMUkrQ6usKnu8UETLon7ifzUvF+/SFg/0ZS9Rm4Or4tS5hSLw31PEkMhs2ryYsSan2SkA==" crossorigin="anonymous" referrerpolicy="no-referrer" />
...
<body class="bg-neutral-50 dark:bg-neutral-900">
    ...
    <div class="flex min-h-screen overflow-x-hidden">
        <!-- Sidebar -->
        <aside id="sidebar" class="fixed left-0 top-0 h-screen w-64 lg:w-72 bg-[#3B7597] text-white shadow-lg transform -translate-x-full transition-transform duration-300 ease-in-out z-50">
            <!-- Sidebar Content -->
            <div class="flex flex-col h-full">
                ...
                <div class="flex items-center justify-between p-6 border-b border-white/10">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center">
                            <i class="fas fa-cube text-[#3B7597] text-lg"></i>
                        </div>
                ...
                <div class="border-t border-white/10 p-4">
                    <div class="flex items-center gap-3 p-3 bg-white/10 hover:bg-white/15 transition-colors cursor-pointer">
                        <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center text-[#3B7597] font-semibold">
                            {{ substr(auth()->user()->name ?? 'U', 0, 1) }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold truncate">{{ auth()->user()->name ?? 'User' }}</p>
                            <p class="text-xs text-white/70 truncate">{{ auth()->user()->email ?? 'user@example.com' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Main Content Area -->
        <div id="mainContent" class="flex-1 flex flex-col w-full lg:w-auto transition-transform duration-300 ease-in-out">
            <!-- Header -->
            <header class="bg-[#0c4366] text-white shadow-md sticky top-0 z-40">
