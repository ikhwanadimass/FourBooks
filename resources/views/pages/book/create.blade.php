<x-layouts::main :title="__('Tambah Buku')">
    <!-- Page Header -->
    <div class="mb-8">
        <a href="{{ route((auth()->user()->role === 'admin' ? 'admin' : 'staff') . '.books.index') }}" class="inline-flex items-center gap-2 text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 mb-4">
            <i class="fas fa-arrow-left"></i>
            Kembali
        </a>
        <h1 class="text-3xl font-bold text-neutral-900 dark:text-white">Tambah Buku Baru</h1>
        <p class="text-neutral-600 dark:text-neutral-400 mt-2">Masukkan informasi buku yang ingin Anda tambahkan</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-neutral-800 rounded-lg shadow-md p-6 lg:p-8 max-w-2xl mx-auto">
        <form method="POST" action="{{ route((auth()->user()->role === 'admin' ? 'admin' : 'staff') . '.books.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input 
                    ...
                    value="{{ old('category') }}"
                    required
                    placeholder="Contoh: Novel, Anak-anak, Pendidikan"
                    class="w-full px-4 py-3 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-white dark:bg-neutral-700 text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-all"
                />
                @error('category')
                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
            ...
            <!-- Author -->
            <div>
                <label for="author" class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                    Pengarang
                </label>
                <input 
                    type="text" 
            ...
            <!-- ISBN -->
            <div>
                <label for="isbn" class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                    ISBN
                </label>
                <input 
                    type="text" 
            ...
            <div class="flex gap-4 pt-6 border-t border-neutral-200 dark:border-neutral-700">
                <a 
                    href="{{ route((auth()->user()->role === 'admin' ? 'admin' : 'staff') . '.books.index') }}"
