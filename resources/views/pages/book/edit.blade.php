<x-layouts::main :title="__('Edit Buku')">
    <!-- Page Header -->
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-neutral-900 dark:text-white">Edit Buku</h1>
        <p class="text-neutral-600 dark:text-neutral-400 mt-2">Perbarui informasi buku Anda</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-neutral-800 rounded-lg shadow-md p-6 lg:p-8 max-w-2xl mx-auto">
        <form method="POST" action="{{ route((auth()->user()->role === 'admin' ? 'admin' : 'staff') . '.books.update', $book->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
