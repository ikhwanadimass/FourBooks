<x-layouts::main :title="__('Produk')">
    <!-- Page Header -->
            ...
            <table class="w-full">
                ...
                <thead>
                    <tr class="bg-neutral-50 dark:bg-neutral-700 border-b border-neutral-200 dark:border-neutral-600">
                        <th class="px-6 py-4 text-left text-sm font-semibold text-neutral-700 dark:text-neutral-300">Produk</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-neutral-700 dark:text-neutral-300">Kategori</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-neutral-700 dark:text-neutral-300">Harga</th>
                ...
                <tbody class="divide-y divide-neutral-200 dark:divide-neutral-600">
                    @forelse($products as $product)
                        <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
                                        <i class="fas fa-leaf text-green-600 dark:text-green-400 text-lg"></i>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-neutral-900 dark:text-white">{{ $product->name }}</p>
                                        <p class="text-xs text-neutral-600 dark:text-neutral-400">{{ $product->name }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <!-- Category -->
                            <td class="px-6 py-4">
                                <span class="text-sm text-neutral-700 dark:text-neutral-300">{{ $product->category }}</span>
                            </td>

                            <!-- Price -->
                            <td class="px-6 py-4">
                                <span class="text-sm font-semibold text-neutral-700 dark:text-neutral-300">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                            </td>

                            <!-- Stock -->
                            <td class="px-6 py-4">
                                <span class="text-sm text-neutral-700 dark:text-neutral-300">{{ $product->stock }}</span>
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    <i class="fas fa-box-open text-4xl text-neutral-300 dark:text-neutral-600 mb-4"></i>
                                    <p class="text-neutral-600 dark:text-neutral-400 text-lg">Belum ada produk</p>
                                    <p class="text-neutral-500 dark:text-neutral-500 text-sm mt-1">Mulai dengan menambahkan produk pertama Anda</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
